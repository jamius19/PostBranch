...
func UpdatePg(ctx context.Context, pg model.Pg) (model.Pg, error) {
	...
	stmt := table.Pg.UPDATE(table.Pg.AllColumns.Except(table.Pg.ID)).
		MODEL(pg).
...
func UpdatePgStatus(ctx context.Context, pgId int32, status, output string) (model.Pg, error) {
	var pg model.Pg

	stmt := table.Pg.UPDATE(table.Pg.Status).
		SET(
			table.Pg.Status.SET(sqlite.String(status)),
